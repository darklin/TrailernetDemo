<template>
  <div class="content-box">
    <div class="content-subtitle">{{$t('contentSubtitle.history.tractionState')}}</div>
    <div class="search-box clearfix">
      <el-form :model="searchForm" class="search-form">
        ...
        <el-form-item :label="$t('trailer.trailerState')" class="fl">
          <el-select v-model="searchForm.trailerState" clearable placeholder="">
            <el-option value="1" :label="$t('trailer.connect')"></el-option>
            <el-option value="0" :label="$t('trailer.notConnect')"></el-option>
          </el-select>
        </el-form-item>
      </el-form>
    ...
    <div class="table-box">
      <el-table
        ...
        <el-table-column
          prop="IsConnectingTrailer"
          align="center"
          :label="$t('trailer.trailerState')">
          <template scope="scope">
            <span style="color: green" v-if="scope.row.IsConnectingTrailer === 1">{{$t('common.yes')}}</span>
            <span style="color: red" v-else-if="scope.row.IsConnectingTrailer === 0">{{$t('common.no')}}</span>
...
export default {
  ...
  methods: {
    ...
    getListData () {
      this.$get({
        url: '/monitor/connectTrailer/listData',
