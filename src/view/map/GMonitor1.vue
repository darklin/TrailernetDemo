  ...
  methods: {
    ...
    handleNodeClick (data, node) {
      if (data.value === 0 || data.Latitude === 0 || data.Longitude === 0) {
        ...
        let point = {lat: data.Latitude, lng: data.Longitude}
    ...
    handleAddress (item, len, i) {
      let point = {lat: item.Latitude, lng: item.Longitude}
      gMap.addEventMarker(point, require(`@/assets/images/${this.markerIcon[item.value]}.png`), item['LicensePlate'], address => {
        ...
        gMap.addEventOverlay(point, content, {
          CarID: item['CarID'],
