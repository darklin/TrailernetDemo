<template>
  <div class="content-box">
    <div class="content-subtitle">{{$t('contentSubtitle.statisticsReport.emptyRateStatistics')}}</div>
    ...
    <div class="table-box">
      <el-table
        ...
        <el-table-column
          prop="emptRate"
          sortable='custom' 
          :label="$t('report.emptyRateStatistics') + ' (%)'">
          <template scope="scope">{{scope.row.emptRate | thousands}}</template>
        </el-table-column>
      </el-table>
...
export default {
  ...
  methods: {
    ...
    initcharts (data) {
      let LicensePlate = []
      let emptRate = []
      let emptRateValue = this.$t('report.emptyRateStatistics') + ' (%)'
      data.forEach(item => {
        LicensePlate.push(item['LicensePlate'])
        emptRate.push(item['emptRate'])
      })
      ...
      let option = {
        ...
        toolbox: {
          ...
          feature: {
            ...
            restore: {show: true},
            saveAsImage: {show: true, name: emptRateValue}
          }
        },
        calculable: true,
        legend: {
    ...
    exportHandle () {
      window.location.href = `${this.$baseURL}emptRate/excel?${this.$serialize(this.searchForm)}`
    },
    sortChange (conditions) {
      if (conditions.prop === 'emptRate') {
        if (conditions.order === 'ascending') {
          tableData.sort(function (a, b) {
            return b.emptRate - a.emptRate
          })
        } else if (conditions.order === 'descending') {
          tableData.sort(function (a, b) {
            return a.emptRate - b.emptRate
          })
        }
      }
