<template>
  <div class="content-box">
    <div class="content-subtitle">{{$t('contentSubtitle.statisticsReport.mileageStatistics')}}</div>
    ...
    <div class="table-box">
      <el-table
        ...
        <el-table-column
          prop="runMilie"
          :label="$t('common.runMilage') + ' (' + unit + ')'"
          sortable='custom' >
          <template scope="scope">{{scope.row.runMilie | thousands}}</template>
        </el-table-column>
        <el-table-column
          prop="totalMilie"
          :label="$t('common.totalRunMileage') + ' (' + unit + ')'"
          sortable='custom' >
          <template scope="scope">{{scope.row.totalMilie | thousands}}</template>
        </el-table-column>
      </el-table>
...
export default {
  ...
  methods: {
    ...
    initcharts (data) {
      let LicensePlate = []
      let runMilie = []
      let runMilieValue = this.$t('common.runMilage') + ' (' + this.unit + ')'
      let totalMilie = []
      let totalMilieValue = this.$t('common.totalRunMileage') + ' (' + this.unit + ')'
      data.forEach(item => {
        LicensePlate.push(item['LicensePlate'])
        runMilie.push(item['runMilie'])
        totalMilie.push(item['totalMilie'])
      })
      ...
      let option = {
        ...
        toolbox: {
          ...
          feature: {
            ...
            restore: {show: true},
            saveAsImage: {show: true, name: this.$t('report.mileageStatistics')}
          },
          iconStyle: {
            normal: {
              borderColor: '#95a2b3'
        ...
        legend: {
          ...
          data: [runMilieValue, totalMilieValue],
          itemGap: 5
        },
        grid: {
          top: '12%',
    ...
    exportHandle () {
      window.location.href = `${this.$baseURL}mileageReport/excel?${this.$serialize(this.searchForm)}`
    },
    sortChange (conditions) {
      if (conditions.prop === 'runMilie') {
        if (conditions.order === 'ascending') {
          tableData.sort(function (a, b) {
            return b.runMilie - a.runMilie
          })
        } else if (conditions.order === 'descending') {
          tableData.sort(function (a, b) {
            return a.runMilie - b.runMilie
          })
        }
      } else if (conditions.prop === 'totalMilie') {
        if (conditions.order === 'ascending') {
          tableData.sort(function (a, b) {
            return b.totalMilie - a.totalMilie
          })
        } else if (conditions.order === 'descending') {
          tableData.sort(function (a, b) {
            return a.totalMilie - b.totalMilie
          })
        }
      }
