<template>
  <div class="content-box">
    <div class="content-subtitle">{{$t('contentSubtitle.statisticsReport.psiStatistics')}}</div>
    ...
    <div class="table-box">
      <el-table
        ...
        <el-table-column
          prop="runTime"
          sortable='custom'
          :label="$t('psi.statistics.PSIRunTime')">
...
export default {
  ...
  methods: {
    ...
    initcharts (data) {
      let LicensePlate = []
      let runTime = []
      let runTimeValue = this.$t('psi.statistics.PSIRunTime')
      data.forEach(item => {
      ...
      let option = {
        ...
        toolbox: {
          ...
          feature: {
            ...
            restore: {show: true},
            saveAsImage: {show: true, name: this.$t('psi.statistics.PSIRunTime')}
          },
          iconStyle: {
            normal: {
              borderColor: '#95a2b3'
    ...
    exportHandle () {
      window.location.href = `${this.$baseURL}psi/report/excel?${this.$serialize(this.searchForm)}`
    },
