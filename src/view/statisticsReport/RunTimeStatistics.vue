<template>
  <div class="content-box">
    <div class="content-subtitle">{{$t('contentSubtitle.statisticsReport.runTimeStatistics')}}</div>
    ...
    <div class="table-box">
      <el-table
        ...
        <el-table-column
          prop="runTime"
          sortable='custom' 
          :label="$t('report.runTimeLen') + ' (h)'">
          <template scope="scope">{{scope.row.runTime | thousands}}</template>
        </el-table-column>
        <el-table-column
          prop="stopTime"
          sortable='custom' 
          :label="$t('report.parkTimeLen') + ' (h)'">
          <template scope="scope">{{scope.row.stopTime | thousands}}</template>
        </el-table-column>
      </el-table>
...
export default {
  ...
  methods: {
    ...
    initcharts (data) {
      let LicensePlate = []
      let runTime = []
      let runTimeValue = this.$t('report.runTimeLen') + ' (h)'
      let stopTime = []
      let stopTimeValue = this.$t('report.parkTimeLen') + ' (h)'
      data.forEach(item => {
        LicensePlate.push(item['LicensePlate'])
        runTime.push(item['runTime'])
        stopTime.push(item['stopTime'])
      })
      ...
      let option = {
        ...
        toolbox: {
          ...
          feature: {
            ...
            restore: {show: true},
            saveAsImage: {show: true, name: this.$t('report.runTimeStatistics')}
          },
          iconStyle: {
            normal: {
              borderColor: '#95a2b3'
        ...
        legend: {
          ...
          data: [runTimeValue, stopTimeValue],
          itemGap: 5
        },
        grid: {
          top: '12%',
    ...
    exportHandle () {
      window.location.href = `${this.$baseURL}runTimeReport/excel?${this.$serialize(this.searchForm)}`
    },
    sortChange (conditions) {
      if (conditions.prop === 'runTime') {
      ...
      } else if (conditions.prop === 'stopTime') {
        if (conditions.order === 'ascending') {
          tableData.sort(function (a, b) {
            return b.stopTime - a.stopTime
          })
        } else if (conditions.order === 'descending') {
          tableData.sort(function (a, b) {
