<template>
  <div class="content-box">
    <el-breadcrumb separator=">">
      ...
      <el-breadcrumb-item>录单基本信息处理</el-breadcrumb-item>
    </el-breadcrumb>
    <div class="info-box">
      <el-tabs v-model="activePane" type="card">
          ...
          <div class="clearfix">
            <div class="write-info-col fl">
              ...
              <el-form ref="custForm" :model="custForm" label-width="100px" class="write-info-form">
                ...
                <el-form-item label="居住开始时间" prop="liveDate" :rules="{ type: 'date', required: true, message: '请选择居住开始时间', trigger: 'change' }" label-width="110px">
                  <el-date-picker
                    ref="custLiveDate"
        ...
        <el-tab-pane label="上传资料" name="second" class="upload-info-box" :disabled="showSecond">
        ...
        <el-tab-pane label="个人征信信息" name="third" class="info-credit-box" :disabled="showThird">
        </el-tab-pane>
      </el-tabs>
    </div>
  </div>
...
export default {
  name: 'write-data',
  data () {
    let customerId = this.$sessionStorage('customerId') || ''
  ...
  created () {
    // 获取申请人城市
    this.$ajax({
      url: '/account/visit/application/getDefaultCityName',
      success: data => {
        this.applyForm['applicantCity'] = data.attr.cityName
      }
    })
    // 获取大类资料
    if (this.customerId) {
