...
export default {
  ...
  methods: {
    // 获取监控数据
    getListTree () {
      console.log(Transform.wgs2bd(22.526049, 113.936006))
      this.$get({
    ...
    initMap (data) {
      if (this.$i18n.locale === 'zh') {
        ...
        for (let [i, len] = [0, data.length - 1]; i <= len; i++) {
          ...
          bMap.addEventMarker(point, require(`@/assets/images/${this.markerIcon[item.value]}.png`), item['LicensePlate'], content, {
            CarID: item['CarID'],
